""" test boundary policy ann02ads-facebook-spi-na-ssm-parameter-rbac-readonly """
...
POLICY_NAME_TO_TEST = "ann02ads-facebook-spi-na-ssm-parameter-rbac-readonly"
...
def get_client(assume_role_arn='', profile_name='default', client_type='secretsmanager', name=random_string_generator(), region=DEFAULT_REGION):
    ...
    if assume_role_arn:
        ...
        print('Please specify profile_name or assume_role_arn')
        sys.exit()
...
class TestPolicySecretManager(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            cls.ssm: botostubs.SSM = get_client(profile_name=PROFILE_TO_TEST, client_type='ssm')
            cls.admin_ssm: botostubs.SSM = get_client(profile_name="default", assume_role_arn=ADMIN_ROLE, client_type="ssm")
        except:
    ...
    def test_get_parameters_bad_name(self):
        random_string = random_string_generator()
        try:
            self.admin_ssm.put_parameter(
                Name=random_string,
                Value=random_string,
                Type='String',
            )
            ...
            self.admin_ssm.delete_parameter(
                Name=random_string,
            )

    def test_get_parameters_good_name(self):
        random_string = random_string_generator(prefix="fbspina-")
        try:
            self.admin_ssm.put_parameter(
                Name=random_string,
                Value=random_string,
