...
execfile('/home/xferuser/multi_client_data_feed/venv/bin/activate_this.py', dict(__file__='/home/xferuser/multi_client_data_feed/venv/bin/activate_this.py'))
...
def get_combine_csv(fn_list, tmp_dir, report_type, dt_range):
    ...
    with open(os.path.join(tmp_dir, fn_out), 'a') as fout:
        writer = csv.writer(fout, delimiter='|', quoting=csv.QUOTE_ALL, lineterminator="\n")
        # first file:
...
def request_report(service, query):
    ...
    try:
        ...
        logger.error("ERROR: execution of the query is failed!: %s" % (error,), exc_info=True)
        return False
...
def download_files(service, report_id, report_fragment, tmp_dir, report_type):
    ...
    fn_local = os.path.join(tmp_dir, report_type + '-' + report_fragment + '.csv')
    ...
    while tries < 10:
        try:
            ...
            logger.error("ERROR: Incomplete read of file from Google Service Report: %s" % (e,), exc_info=True)
            tries += 1
            time.sleep(30)
    f.close()
        ...
        try:
            ...
            if json_data['isReportReady']:
                ...
                for i in range(len(json_data['files'])):
                    logger.info('Downloading fragment ' + str(i) + ' for ' + report_type + ' report ' + report_id)
                    fragment = download_files(service, report_id, str(i), tmp_dir, report_type) # See Download the report.
                    fragments_fn.append(fragment)

                dt_range = dt_start + '_' + dt_end if dt_start != dt_end else dt_start
                fn_report = get_combine_csv(fragments_fn, tmp_dir, report_type, dt_range)


                fn_ctrl = fn_report.split('.')[0] + '.ctl'
                with open(os.path.join(tmp_dir, fn_ctrl), 'w') as fp:
                    print >> fp, '%s|%s|%d|%d' % (fn_report, dt_range.replace('-', ''), os.stat(os.path.join(tmp_dir, fn_report)).st_size, json_data['rowCount'])
                sftp_batch = os.path.join(tmp_dir, 'sftp_batch_file')
                with open(sftp_batch, 'w') as fp:
                    print >> fp, 'put %s\nbye' % (os.path.join(tmp_dir, fn_report),)
                ...
                if ret['exitcode']:
                    logger.error("ERROR: transfering %s from local to wf-prod via scp is failed!" % (fn_report,), exc_info=True)
                    raise Exception("BOOM")
                else:
                    os.remove(os.path.join(tmp_dir, fn_report))
                sftp_batch = os.path.join(tmp_dir, 'sftp_batch_file')
                with open(sftp_batch, 'w') as fp:
                    print >> fp, 'put %s\nbye' % (os.path.join(tmp_dir, fn_ctrl),)
                ret = run_command('sftp -C -b %s wf-prod' % (sftp_batch,))
                #ret = run_command('scp %s localhost:/vol/data/mock-sftp/ds3/' % (os.path.join(tmp_dir, fn_ctrl),))
                if ret['exitcode']:
                    logger.error("ERROR: transfering %s from local to wf-prod via scp is failed!" % (fn_ctrl,), exc_info=True)
                    raise Exception("BOOM")
                else:
            ...
            # See Response Codes
            logger.error('HTTP code %d, reason %s' % (e.resp.status, error['reason']))
            break
...
def do_work(start_date):
    for query_template in QUERY_TEMPLATES:
        ...
        creds = create_credentials(SETTINGS["client_id"], SETTINGS["client_secret"], SETTINGS["refresh_token"])
        service = get_service(creds)
        report = request_report(service, QUERY)
        if report:
            report_id = report
            poll_report(service, report_id, QUERY['reportType'], dt_start=QUERY['timeRange']['startDate'], dt_end=QUERY['timeRange']['endDate'])
...
if __name__=="__main__":
    ...
    parser.add_argument("-d", "--date_needed", required=True, help="Input date needed in format: YYYY-MM-DD")
    parser.add_argument("-q", "--query_template", required=True, choices=["keyword", "visit", "conversion"], action="append", help="Query template. Ex. -q conversion,visit")
