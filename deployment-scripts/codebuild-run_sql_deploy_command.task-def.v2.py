...
def get_task_environment_batch(jobDefinitionArn) -> dict:
    ...
    environment = response['jobDefinitions'][0]['containerProperties']['environment']
...
def get_job_definition_arn():
    """Returns task_definition_arn"""

    compute_engine = 'fargate'

    job_definition_arn = os.environ.get('JOB_DEFINITION_NAME') or f'{APP_NAME}-{compute_engine}-ecs-job-defn'

    print(f'Using {job_definition_arn}')

    return job_definition_arn

def get_job_definition_arn_batch():
    """Returns task_definition_arn"""

    compute_engine = 'fargate'

    job_definition_arn = os.environ.get('JOB_DEFINITION_NAME') or f'{APP_NAME}-{compute_engine}-batch-job-defn'

    print(f'Using {job_definition_arn}')

    return job_definition_arn

def execute_sql_deploy_command_in_batch() -> str:
    """ Executes sql_deploy_command using batch.submit_job() """


    jobName = f'{APP_NAME}-SQL-DEPLOY'
    jobDefinitionArn = get_job_definition_arn_batch()

    jobQueue_default = 'infra-fargate-job-queue'
    jobQueue = os.environ.get('JOB_QUEUE_NAME') or jobQueue_default

    environment = get_task_environment_batch(jobDefinitionArn)
    # launch_type = task_definition['launchType']

    for i, item in enumerate(environment):
        print(item)
        if item['name'] == 'LAMBDA_FUNCTION_NAME':
            environment[i]['value'] = ''
            break
    else:
        # avoiding lamda-entrypoint unbound var error
        environment.append({'name': 'LAMBDA_FUNCTION_NAME', 'value' : ''})

    for i, item in enumerate(environment):
        print(item)
        if item['name'] == 'AWS_LAMBDA_RUNTIME_API':
            environment[i]['value'] = ''
            break
    else:
        # avoiding lamda-entrypoint unbound var error
        environment.append({'name': 'AWS_LAMBDA_RUNTIME_API', 'value' : ''})
...
def monitor_task(task_arn: str) -> bool:
    ...
    while task_description is None or last_status != 'STOPPED':
        task_description = client.describe_jobs(
            cluster=ECS_CLUSTER,
            tasks=[
                task_arn,
            ],
...
def get_sql_deploy_command() -> list:
    ...
    sql_deploy_command = os.environ.get("SQL_DEPLOY_COMMAND")
...
def main():
    submit_job_ouput = execute_sql_deploy_command_in_batch()

    print(json.dumps(obj=submit_job_ouput, indent=4, default=str))
    print('Please check log status.')

    # job_arn = monitor_task(submit_job_ouput['jobArn'])
    ...
    try:
        ...
        if DEPLOY_ROLE:
            ...
            response = client.assume_role(
                RoleArn=DEPLOY_ROLE,
                RoleSessionName='deploy-task-session'
            )
